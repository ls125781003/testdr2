...
async function main() {
    let js_order = ['豆瓣┃搜索', '玩偶哥哥┃网盘配置', '360影视[官]', '菜狗[官]', '奇珍异兽[官]', '优酷[官]', '腾云驾雾[官]', '百忙无果[官]', '哔哩影视[官]', '哔哩教育[官]', '哔哩大全[官]', '直播转点播[合]'];
    let js_path = './drpy_js';
    let live_path = './lives';
    ...
    channels.push({
        'name': '稳定github直播',
        'urls': [
            'proxy://do=live&type=txt&ext=https://gh-proxy.com/https://raw.githubusercontent.com/hjdhnx/hipy-sniffer/refs/heads/main/static/lives/lives.txt',
        ],
    });
    live_files.forEach((it) => {
        let absp = pathLib.join(path_dir, `${live_path}/${it}`).replace(/\\/g, '/');
        if (absp.includes('/zyfun/file/')) {
            absp = 'http://127.0.0.1:9978/api/v1/file/' + absp.split('/zyfun/file/')[1];
        }
